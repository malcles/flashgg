...
int TagTruthBase::HTXSstage0orderedBin() const {
    auto theIt = stage0map_.find(stage0cat_);
    if( theIt != stage0map_.end() ) {
        return theIt->second;
    }
    else {
        throw cms::Exception( "Missing Data" ) << "Could not find value (in the TagTruthBase) for the STXS process: " << stage0cat_ << "\n";;
    }
}

int TagTruthBase::HTXSstage1orderedBin() const {
    auto theIt = stage1map_.find(stage1cat_);
    if( theIt != stage1map_.end() ) {
        return theIt->second;
    }
    else {
        throw cms::Exception( "Missing Data" ) << "Could not find value (in the TagTruthBase) for the STXS process: " << stage1cat_ << "\n";;
    }
}
...
float TagTruthBase::HTXSggHweight( std::string weightName ) const {
    auto theIt = ggHweights_.find(weightName);
    if( theIt != ggHweights_.end() ) {
        return theIt->second;
    }
    else {
        throw cms::Exception( "Missing Data" ) << "Could not find value (in the TagTruthBase) for the ggH weight called: " << weightName << "\n";
    }
}
